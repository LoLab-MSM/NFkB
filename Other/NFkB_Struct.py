...
Parameter('KNN', 2.0e5)
Parameter('KN', 1.0e5) #10e4
Parameter('M', 2000) #10e3
Parameter('AN', 2)
Parameter('ANa', 2)
...
Initial(TNF_ext(), Parameter('TNF_ext_0', 10))
...
# Parameter('kb', 1.2e-5) #receptor activation rate 4e-6
# Parameter('kf', 1.2e-3) #receptor inactivation rate 6e-4
# Parameter('Tdeg', 7.7e-4) #TNF loss same as cdeg
# Parameter('ka',1e-5) #IKKK kinase activation rate #10e-4
# Parameter('ka20', 1e5) #A20 TNFR1 block #1e4
# Parameter('ki', 0.01) #IKKK kinase inactivation rate
# Parameter('k1', 2.0*6e-10) #IKKn activation by IKKK #5e-6
# Parameter('k3', 0.002) #IKKa inactivation by A20 #0.003
# Parameter('k2', 10000) #IKKa inactivation
# Parameter('k4', 0.001) #IKKii transfer rate #0.0005
...
# Parameter('q1', 4e-7) #NFkB attaching @ A20 and IkBa site 1.5e-7
...
#Declaring Parameters
Parameter('kv', 5.0) #Nucealr to cytoplasm volume
Parameter('kb', 4e-6) #receptor activation rate
Parameter('kf', 6e-4) #receptor inactivation rate
Parameter('Tdeg', 7.7e-4) #TNF loss same as cdeg
Parameter('ka',10e-4) #IKKK kinase activation rate
Parameter('ka20', 1e4) #A20 TNFR1 block
Parameter('ki', 0.01) #IKKK kinase inactivation rate
Parameter('k1', 5e-6) #IKKn activation by IKKK
Parameter('k3', 0.003) #IKKa inactivation by A20
Parameter('k2', 10000) #IKKa inactivation
Parameter('k4', 0.0005) #IKKii transfer rate
Parameter('c1a', 0.1) #inducible IkBa mRNA synthesis
Parameter('a2', 1e-7) #IkBa phosphorylation b/c IKKa
Parameter('tp', 0.01) #degradation of phosph-IkBa complex with NFkB
Parameter('a3', 5e-7) #IkBa_NFkB phosphorylation b/c IKKa
...
Parameter('c1', 0.1) #inducible A20 mRNA synthesis
Parameter('c3', 0.00075) #A20 and IkBa mRNA deg rate
Parameter('q1', 1.5e-7) #NFkB attaching @ A20 and IkBa site
Parameter('q2', 1e-6) #IkBa inducible detaching from A20, IkBa site
Parameter('k3_div_k2', k3.value/k2.value) #for IKKa and A20
Parameter('a1_mult_kv', a1.value*kv.value) #for volume IkBa association NFkB
...
tspan = np.linspace(0, 36000, 36000)
x = BngSimulator(model, tspan) #integrator='lsoda',
simulation_result = x.run(method='ode')

# plt.figure()
# plt.plot(time/3600, result.observables["NFkBn_obs"], label='NFkBn')
# plt.xlabel("Time (in min)", fontsize=16)
# plt.ylabel("Concentration uM", fontsize=16)
# plt.legend()
# #
# plt.figure()
# plt.plot(time/3600, result.observables["IKKa_obs"], label='Ikka')
# plt.xlabel("Time (in minutes)", fontsize=16)
# plt.ylabel("Concentration uM", fontsize=16)
# plt.legend()
#
# plt.figure()
# plt.plot(time/3600, result.observables["IKKKa_obs"], label='Ikkka')
# plt.xlabel("Time (in minutes)", fontsize=16)
# plt.ylabel("Concentration uM", fontsize=16)
# plt.legend()
# #
# plt.figure()
# plt.plot(time/3600, result.observables["A20t_obs"], label='A20t')
# plt.xlabel("Time (in minutes)", fontsize=16)
...
# plt.figure()
plt.subplot(231)
plt.plot(tspan/3600, simulation_result.observables['TNF_ext_obs'],label = 'TNF')
...
# plt.figure()
plt.subplot(232)
plt.plot(tspan/3600, simulation_result.observables['NFkBn_obs'],label = 'NFkBn')
...
# plt.figure()
plt.subplot(233)
plt.plot(tspan/3600, simulation_result.observables['IKKa_obs'],label = 'IKKa')
...
plt.subplot(234)
plt.plot(tspan/3600, simulation_result.observables['IkBa_tot'], label = 'IkBa_tot')
...
# plt.figure()
plt.subplot(235)
plt.plot(tspan/3600, simulation_result.observables['A20t_obs'], label = 'A20t')
...
# plt.figure()
plt.subplot(236)
plt.plot(tspan/3600, simulation_result.observables['A20_obs'], label = 'A20')
