...
Observable('L_1', L(loc='cell_1')**ECM)
Observable('L_2', L(loc='cell_2')**ECM)
        
Observable('R_1', R()**CellMem1)
Observable('R_2', R()**CellMem2)
...
Parameter('pNGN1_0', 100)
Parameter('pASCL2_0', 200)
...
Initial(R(l=None,ysc=None)**CellMem1, R_1_0)
Initial(R(l=None,ysc=None)**CellMem2, R_2_0)
Initial(YSC(lr = None)**CellMem1, YSC1_0)
Initial(YSC(lr = None)**CellMem2, YSC2_0)
Initial(CSL(icd = None)**Nuc1, CSL1_0)
Initial(CSL(icd = None)**Nuc2, CSL2_0)
Initial(pASCL1()**Cyto1, pASCL1_0)
Initial(pASCL1()**Cyto2, pASCL2_0)
Initial(pNGN2()**Cyto1, pNGN1_0)
...
Parameter('kLform', 0.1)
Parameter('kLRf', 0.1)
Parameter('kLRr', 0.001)
Parameter('kLRYSCf', 0.1)
Parameter('kLRYSCr', 0.001)
Parameter('kYSCICD', 1e-2)

#Ligand of one cell binds receptor of another

Rule('L_1_binds_R_2', L(r=None,loc='cell_1') **ECM + R(l=None,ysc=None)**CellMem2 <> 
        L(r=1,loc='cell_1') **ECM % R(l=1,ysc=None)**CellMem2 , kLRf,kLRr)


Rule('L_2_binds_R_1', L(r=None,loc='cell_2') **ECM + R(l=None,ysc=None)**CellMem1 <> 
        L(r=1,loc='cell_2') **ECM % R(l=1,ysc=None)**CellMem1 , kLRf,kLRr)

Rule('YSC_binding', YSC(lr=None) + R(l=ANY,ysc=None) <>
                     YSC(lr=1) % R(l=ANY,ysc=1), kLRYSCf,kLRYSCr)


Rule('ICD_formation_1',L(r=1, loc='cell_1') ** ECM % R(l=1, ysc=2) ** CellMem2 % YSC(lr=2) ** CellMem2 
     >>R_ICD(csl=None)**Cyto2 + YSC(lr=None)**CellMem2 + L_endo()**Cyto1, kYSCICD)
Rule('ICD_formation_2',L(r=1, loc='cell_2') ** ECM % R(l=1, ysc=2) ** CellMem1 % YSC(lr=2) ** CellMem1 
     >>R_ICD(csl=None)**Cyto1 + YSC(lr=None)**CellMem1 + L_endo()**Cyto2, kYSCICD)
...
Rule('ICD_cyto_to_nucleus_1', R_ICD(csl=None)**Cyto1 >> R_ICD(csl=None)**Nuc1, kICD_c_n)
Rule('ICD_cyto_to_nucleus_2', R_ICD(csl=None)**Cyto2 >> R_ICD(csl=None)**Nuc2, kICD_c_n)

Rule('ICD_CSL_complex', R_ICD(csl = None) + CSL(icd = None) <> R_ICD(csl = 1)%CSL(icd = 1), kICDCSLf, kICDCSLr)
...
Rule('Receptor_decay', R() >> None, d_R)
Rule('ICD_decay', R_ICD() >> None, d_ICD)
...
Rule('R_synthesis_1', None >> R(l=None,ysc=None)**CellMem1, kRsynth)
Rule('R_synthesis_2', None >> R(l=None,ysc=None)**CellMem2, kRsynth)
#
