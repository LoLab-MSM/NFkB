...
Initial(IKK(state = 'ii'), Parameter('IKKii', KNN.value-IKKn_0.value-IKKa_0.value-IKKi_0.value))
...
Observable('A20_gene_all_obs', A20_gene(state = 'on')+A20_gene(state ='off'))
Observable('IkBa_gene_all_obs', IkBa_gene(state = 'on')+IkBa_gene(state ='off'))
...
plt.figure(1)
for obs in ["IKKKa_obs", "IKKii_obs", "IKKa_obs", "IKKi_obs", "IkBap_obs", "IkBan_NFkBn_obs"]: # ["A20_obs", "IKKii_obs"]: #, "A20_off_obs", "A20t_obs"]:
    plt.plot(time/60, x[obs], label=re.match(r"(\w+)_obs", obs).group(), linewidth=3)
    # plt.subplot(2,1,1)
    # plt.plot(time/60, x["A20_obs"], label = 'A20')
    # plt.subplot(2,1,2)
    # plt.plot(time/60, x["IKKii_obs"], label = 'IKKii')
    plt.legend(loc=0, prop={'size': 16})
    plt.xlabel("Time (in minutes)", fontsize=16)
...
plt.figure()
for obs in ["IkBa_on_obs", "IkBa_off_obs", "IkBat_obs", "IkBa_gene_all_obs"]:
    plt.plot(time/60, x[obs], label=re.match(r"(\w+)_obs", obs).group(), linewidth=3)
    # plt.figure(2)
    # plt.subplot(2,1,1)
    # plt.plot(time/60, x["IkBa_on_obs"], label = 'IkBaon')
    # plt.subplot(2,1,2)
    # plt.plot(time/60, x["IkBat_obs"], label = 'IkBa_mRNA')
    plt.legend(loc=0, prop={'size': 16})
    plt.xlabel("Time (in minutes)", fontsize=16)
    plt.ylabel("Concentrations", fontsize=16)

plt.figure()
for obs in ["A20_on_obs", "A20_off_obs", "A20t_obs", "A20_gene_all_obs"]:
    plt.plot(time/60, x[obs], label=re.match(r"(\w+)_obs", obs).group(), linewidth=3)
    # plt.figure(2)
    # plt.subplot(2,1,1)
    # plt.plot(time/60, x["IkBa_on_obs"], label = 'IkBaon')
